...
namespace FlexiMvvm.Navigation
{
    ...
    public sealed class ViewControllerBackwardNavigationStrategy
    {
        ...
        private ILogger Logger => _logger ??= FlexiMvvmConfig.Instance.GetLoggerFactory().CreateLogger<ViewControllerBackwardNavigationStrategy>();
        ...
        public ViewControllerBackwardNavigationDelegate PopViewController()
        {
            return (sourceView, animated) =>
            {
                if (sourceView.NavigationController != null)
                {
                ...
                {
                    Logger.LogDebug(
                        $"Unable to pop '{TypeFormatter.FormatName(sourceView.GetType())}' instance due to " +
                        $"'{TypeFormatter.FormatName(sourceView.GetType())}.{nameof(UIViewController.NavigationController)}' is 'null'.");
                }
            };
        }
        ...
        public ViewControllerBackwardNavigationDelegate PopToViewController(UIViewController targetView)
        {
            ...
            return (sourceView, animated) =>
            {
                ...
                {
                    Logger.LogDebug(
                        $"Unable to pop to '{TypeFormatter.FormatName(targetView.GetType())}' instance due to " +
                        $"'{TypeFormatter.FormatName(sourceView.GetType())}.{nameof(UIViewController.NavigationController)}' is 'null'.");
                }
            };
        }
        ...
        public ViewControllerBackwardNavigationDelegate PopToRootViewController()
        {
            return (sourceView, animated) =>
            {
                ...
                {
                    Logger.LogDebug(
                        $"Unable to pop to root view controller due to " +
                        $"'{TypeFormatter.FormatName(sourceView.GetType())}.{nameof(UIViewController.NavigationController)}' is 'null'.");
                }
            };
        }
